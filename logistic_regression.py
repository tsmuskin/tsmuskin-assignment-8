...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        model = LogisticRegression()
        model.fit(X, y)
        # Record the coefficients (beta0, beta1, beta2)
        beta0, beta1, beta2 = model.intercept_[0], model.coef_[0][0], model.coef_[0][1]
        beta0_list.append(beta0)
        beta1_list.append(beta1)
        beta2_list.append(beta2)
        # Record the slope and intercept of the decision boundary
        slope = -beta1 / beta2  # slope = -beta1/beta2 (from the decision boundary equation)
        intercept = -beta0 / beta2  # intercept = -beta0/beta2
        ...
        def compute_log_loss(model, X, y):
            # Get predicted probabilities for the positive class (class 1)
            y_pred_proba = model.predict_proba(X)[:, 1]  # Assuming the model has a method predict_proba
            
            # Compute log loss
            epsilon = 1e-15  # To avoid log(0) error
            y_pred_proba = np.clip(y_pred_proba, epsilon, 1 - epsilon)  # Clip the probabilities to prevent log(0)
            
            log_loss = -np.mean(y * np.log(y_pred_proba) + (1 - y) * np.log(1 - y_pred_proba))
            return log_loss
        
        log_loss = compute_log_loss(model,X, y)  # We need to calculate the logistic loss
        ...
        plt.contour(xx, yy, Z, levels=[0.5], cmap="gray", linestyles="--")
        plt.plot(xx[:, 0], slope * xx[:, 0] + intercept, 'r-', label=f"Decision Boundary: y = {slope:.2f} * x + {intercept:.2f}")
    ...
    plt.subplot(3, 3, 4)
    slope_list = [b1 / b2 if b2 != 0 else np.nan for b1, b2 in zip(beta1_list, beta2_list)]  # Prevent division by zero
    ...
    plt.subplot(3, 3, 5)
    intercept_ratio = [b0 / b2 if b2 != 0 else np.nan for b0, b2 in zip(beta0_list, beta2_list)]  # Prevent division by zero
    plt.plot(shift_distances, intercept_ratio, marker='o', linestyle='-', color='orange')
